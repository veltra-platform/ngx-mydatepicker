...
*  - Add html template and styles as inline templates to the ngx-my-date-picker.component.
...
gulp.task('backup.component.tmp', function() {
    return gulp.src('./src/ngx-my-date-picker/ngx-my-date-picker.component.ts').pipe(gulp.dest('./tmp'));
});

gulp.task('minify.css', function() {
    return gulp.src('./src/ngx-my-date-picker/ngx-my-date-picker.component.css')
        .pipe(cleancss({compatibility: 'ie8'}))
        .pipe(gulp.dest('./tmp'));
});

gulp.task('minify.html', function() {
    return gulp.src('./src/ngx-my-date-picker/ngx-my-date-picker.component.html')
        .pipe(htmlmin({collapseWhitespace: true, caseSensitive: true}))
        .pipe(gulp.dest('./tmp'));
});

gulp.task('inline.template.and.styles.to.component', function() {
    var styles = fs.readFileSync('./tmp/ngx-my-date-picker.component.css', 'utf-8');
    var htmlTpl = fs.readFileSync('./tmp/ngx-my-date-picker.component.html', 'utf-8');

    styles = styles.split('\\e').join('\\\\e');

    return gulp.src(['./src/ngx-my-date-picker/ngx-my-date-picker.component.ts'])
...
gulp.task('delete.modified.component', function () {
    return gulp.src(['./src/ngx-my-date-picker/ngx-my-date-picker.component.ts'], {read: false}).pipe(clean());
});

gulp.task('restore.original.component', function() {
    return gulp.src('./tmp/ngx-my-date-picker.component.ts').pipe(gulp.dest('./src/ngx-my-date-picker/'));
});
...
gulp.task('tslint', function () {
    return gulp.src([
        "src/ngx-my-date-picker/directives/*.ts",
        "src/ngx-my-date-picker/interfaces/*.ts",
        "src/ngx-my-date-picker/services/*.ts",
        "src/ngx-my-date-picker/index.ts",
        "src/ngx-my-date-picker/ngx-my-date-picker.input.ts",
        "src/ngx-my-date-picker/ngx-my-date-picker.component.ts",
        "src/ngx-my-date-picker/ngx-my-date-picker.module.ts"])
        .pipe(tslint({
